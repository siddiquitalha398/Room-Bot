...
ROOM_ID = "69e6d005fb01892e49d4c382"
API_TOKEN = "66f6d9b4c0c9bea4cb0f062c509a1413dc64ba4ba9c250cdfcd1475c14186604"
...
class Bot(BaseBot):
    ...
    def __init__(self):
        ...
        # Spawn positions
        self.floor1_position    = Position(12.0, 12.0, 9.0,  facing="FrontRight")
        self.vip_spawn_points   = [Position(12.0, 22.0, 8.0,  facing="FrontRight")]
        self.dj_spawn_point     = Position(9.0,  22.0, 27.0, facing="FrontRight")
        self.ground_spawn_position = Position(12.0, 12.0, 9.0, facing="FrontRight")

        self.active_emote_loops = {}
        self.tip_queue = asyncio.Queue()
        self.room_stay_tracker = {}
        self.welcome_in_progress = set()
    ...
    def load_database_file(self) -> None:
        if not os.path.exists(DATA_FILE):
            try:
                with open(DATA_FILE, "w") as file:
                    dump({"users": {}, "vip_users": [], "dj_users": [], "welcome_payouts": [], "bot_position": {"x": 0, "y": 0, "z": 0, "facing": "FrontRight"}}, file)
        ...
        try:
            with open(DATA_FILE, "r") as file:
                ...
                self.dj_users = data.get("dj_users", [])
                self.welcome_payouts = data.get("welcome_payouts", [])
                print(f"[MEMORY LOG] Loaded Brain. Tippers={len(self.tip_data)}, VIPs={len(self.vip_users)}, DJs={len(self.dj_users)}")
        except Exception as e:
    ...
    async def command_handler(self, user: User, message: str, source: str):
        ...
        is_dj = (user.id in self.dj_users)

        # 3. Handle Emote Triggering by number (all users)
        try:
        ...
        # 4. Standard Commands (all users)
        if clean_msg == "!help":
            help_text = "⚡ Commands: !list | !stop | F1"
            if is_vip or is_owner:
                help_text += " | !vip | !down | !bring @username"
            if is_dj or is_owner:
                help_text += " | !dj"
            if is_owner:
                help_text += " | !owner | !set | !top | !bal | !allvips | !alldjs | !giveall | !give | !givevip | !removevip | !givedj | !removedj | !giveowner"
        ...
        elif clean_msg in ("f1", "!f1"):
            # All users including DJ, VIP, owner, normal users
            try:
        ...
        elif clean_msg == "!dj" and (is_dj or is_owner):
            try:
                await self.highrise.teleport(user.id, self.dj_spawn_point)
            except Exception:
                pass
            return
        ...
        if clean_msg == "!top":
            ...
            try:
                ...
                for u, _ in room_users.content:
                    if u.username.lower() == target_name:
                        if u.id not in self.dj_users:
                            self.dj_users.append(u.id)
                            if u.id not in self.tip_data:
                                self.tip_data[u.id] = {"username": u.username, "total_tips": 0}
                            self.save_database_file()
                            await self.respond(user, f"🎧 @{u.username} has been granted DJ access!", source)
                        else:
                            await self.respond(user, f"⚠️ @{u.username} is already a DJ.", source)
                        return
                await self.respond(user, "❌ User not found in the room.", source)
            except IndexError:
            ...
            try:
                ...
                for u, _ in room_users.content:
                    if u.username.lower() == target_name:
                        if u.id in self.dj_users:
                            self.dj_users.remove(u.id)
                            self.save_database_file()
                            await self.respond(user, f"🚫 @{u.username} has had their DJ access revoked.", source)
                        else:
                            await self.respond(user, f"⚠️ @{u.username} is not a DJ.", source)
                        return
                await self.respond(user, "❌ User not found in the room.", source)
            except IndexError:
        ...
        elif clean_msg == "!alldjs":
            if not self.dj_users:
                await self.highrise.send_whisper(user.id, "No DJs found in database.")
                return
            dj_names = []
            for dj_id in self.dj_users:
                name = self.tip_data.get(dj_id, {}).get("username", "Unknown User")
                dj_names.append(name)
            dj_string = ", ".join(dj_names)
            await self.highrise.send_whisper(user.id, f"🎧 Total DJs ({len(dj_names)}): {dj_string}")
            return
